	...
	//Onur Her þey bu if'in içinde oluyor
		...
		*/
		...
		if( ( (runs > timeToGoInit+wait) && (trialNo==1) ) || (trialNo!=1) )
			...
			*/
			...
			double forceConstant = 0.0005f;
			force2[0] = hapticForce.at(0) * forceConstant;
			force2[2] = hapticForce.at(1) * forceConstant;
			//
		...
		*/
		...
		if(((runs>timeToGoInit+wait)&&(trialNo==1)) || (trialNo!=1))
		{
			...
			//float forceConstant = 1.0f;
			//force2[0] = forceConstant * hapticForce.at(0);
			//force2[1] = forceConstant * hapticForce.at(1);
...
Vector calcBallPos()
	...
	int res_x = ( int(bpZ) - Zsrc_min) / (Zsrc_max - Zsrc_min) * ( Xres_max - Xres_min) + Xres_min;
	int res_y = ( int(bpX) - Xsrc_min) / (Xsrc_max - Xsrc_min) * ( Yres_max - Yres_min) + Yres_min;


	//hapticForce = path.calculatePath(res_x,res_y);
	...
	Zres_min = -41;
	*/
	//int oyunballX = (int(bpX) - Xsrc_min) / (Xsrc_max - Xsrc_min) * (Xres_max - Xres_min) + Xres_min;
	//int oyunballY = (int(bpZ) - Zsrc_min) / (Zsrc_max - Zsrc_min) * (Zres_max - Zres_min) + Zres_min;

	//if(runs%1000 == 0){

	//cout << "path.targetx is: " << path.targetx << " path.targety is: " << path.targety << "\n"; 
	//cout << "oyunballX is: " << oyunballX << " oyunballY is: " << oyunballY << "\n"; 
	//cout << "OyunTargetX is: " << oyunTargetX << " OyunTargetZ is: " << oyunTargetY << "\n";
	//}
	...
	if(runs % 1000 == 0 ){
		//cout << "bpX is: " << bpX << "\tbpY is: " << bpZ << "\n"; 
		//cout << "res_x is: " << res_x << "\tres_y is: " << res_y << "\n";
		//cout << "path.targetx is: " << path.targetx << " path.targety is: " << path.targety << "\n";
		}
	//DOðrusu sanýrým bu
	hapticForce = path.calculatePath(res_x,res_y);
	...
	if(runs % 1000  == 0){
		...
		cout << "algorithm x force is: " << hapticForce.at(1) * forceConstant << "algorithm z force is: " << hapticForce.at(0) * forceConstant << "\n";
		cout << "haptic x force is: " << kpHN*(hpX-hhpX) + kdN*(hvX-hhvX) << " haptic y force is: "  << kpHN*(hpZ-hhpZ) + kdN*(hvZ-hhvZ) << "\n"; 
		}

	//Onur

	forceBall[0] = kpHN*(hpX-hhpX) + kdN*(hvX-hhvX) 
		+ /*Onur Right(Device HHD2 ) deviceden gelen force u 0 la yerine benim algorithmamýn ürettiði forcu koy kpHN*(cpX-hcpX) + kdN*(cvX-hcvX)*/ (  hapticForce.at(1) * forceConstant) +fcw[0] + boundaryforceCX;

	forceBall[2] = kpHN*(hpZ-hhpZ) + kdN*(hvZ-hhvZ)
		+/*Onur Right(Device HHD2 ) deviceden gelen force u 0 la yerine benim algorithmamýn ürettiði forcu koy  kpHN*(cpZ-hcpZ) + kdN*(cvZ-hcvZ)*/ ( hapticForce.at(0) * forceConstant) +fcw[2] + boundaryforceCZ;
