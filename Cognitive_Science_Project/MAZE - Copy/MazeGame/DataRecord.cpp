...
//	}
	...
	for(unsigned int i = 0; i < runsCnt->size(); i++){
		...
		fileOutPos	
				...
				<< trialID->at(i)			<< "\t"	<<	runsCnt->at(i)			<< "\t"      // 1
				<< theta->at(i)			    << "\t" <<	fOnCIP->at(i)[0]			<< "\t"	// 2
				<< fOnCIP->at(i)[2]		    << "\t" <<	fOnHIP->at(i)[0]		<< "\t"		// 3
				/*<< tmpFBI[0]				<< "\t" <<	tmpFBI[2]				<< "\t"
				<< tmpFBS[0]				<< "\t" <<	tmpFBS[2]				<< "\t"*/
				<< fOnHIP->at(i)[2]		    << "\t"// <<	fOnHIP->at(i)[2]		<< "\t"		// 4

				<< ballP->at(i)[0]			<< "\t" <<	ballP->at(i)[2]			<< "\t"		// 5
				<< ballV->at(i)[0]			<< "\t" <<	ballV->at(i)[2]			<< "\t"		// 6
				<< ballA->at(i)[0]			<< "\t" <<	ballA->at(i)[2]			<< "\t"		// 7
				<< cipP->at(i)[0]			<< "\t" <<	cipP->at(i)[2]			<< "\t"		// 8
				<< cipV->at(i)[0]			<< "\t" <<	cipV->at(i)[2]			<< "\t"		// 9
				<< hipP->at(i)[0]			<< "\t" <<	hipP->at(i)[2]			<< "\t"		// 10
				<< hipV->at(i)[0]			<< "\t" <<	hipV->at(i)[2]			<< "\t"		// 11
				<< timeScore->at(i)	<<endl;
