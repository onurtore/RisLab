...
std::string wheretoGo::ConvertO(float number){
...
int wheretoGo::foundPaths(){
    ...
    string file_string1 = "0_path.txt";
    string file_string2 = "1_path.txt";
...
void wheretoGo::generatePaths(){
    ...
    //New iteration for every target point
    for(int i = 0 ; i < 2 ; i++ ){
        switch(i){
            case 0:
                cout << "Generating first path\n";
                addStartPoint(40, 38);
                addTargetPoint(13, 176);
                break;
            case 1:
                cout << "Generating second path\n";
...
void wheretoGo::sumMatrices(int i)
{
    ...
    string file_string = ConvertO(i) + "sumMatrices.txt";
...
void wheretoGo::AddPotentialField(int i){
    ...
    string file_string = ConvertO(i) + "PotantielField.txt";
...
void wheretoGo::calculateWavefront(int i ) {
        ...
        string file_string; 
        file_string = ConvertO(i) + "WaveFront.txt";
...
void wheretoGo::calculatePath(int no) {
    ...
    string file_string = ConvertO(no) + "FirstPath.txt";
...
void wheretoGo::printPath(int i ){

    ofstream out;
    string file_string = ConvertO(i) + "_path.txt";

    out.open(file_string);


    for(int j = 0 ; j  < dtargetsX.size(); j++){
        out << dtargetsX.at(j) << '\t' << dtargetsY.at(j) << '\n';
    }
    out.close();
...
int wheretoGo::borderOut() {
	...
	if (  (abs(startx) > matrixHeight) || (abs(starty) > matrixWidth) )
	{
