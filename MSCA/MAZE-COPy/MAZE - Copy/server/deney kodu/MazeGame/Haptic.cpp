...
vector<float> cipPos;
	...
	//Onur -> Calculate the force for the drag the  cube (ball), to the neareast point, on the path
		...
		*/
		...
		static bool OnurfirstTime = true;
		
		if(bpX < -61){
			OnurfirstTime = false;
		}

		for(int k = 0 ; k < path.dtargetsX.size(); k++){

					double res_z = ((path.dtargetsX.at(k) - ECXsrc_min) / (ECXsrc_max - ECXsrc_min) * (ECZres_max - ECZres_min) + ECZres_min);
					double res_x = ((path.dtargetsY.at(k) - ECYsrc_min) / (ECYsrc_max - ECYsrc_min) * (ECXres_max - ECXres_min) + ECXres_min);
	...
	//Not the first time
	//Onur Her �ey bu if'in i�inde oluyor
...
Vector calcBallPos(vector<float> cip_pos)
{
	...
	string ss  = ConvertO(forceBall[0]) + '\t' + ConvertO(forceBall[2]) + '\t';
	...
	static_friction_magnitude = ballMass * G * COEFFICIENT_FRICTION_STATIC;
	kinetic_friction_magnitude = ballMass * G * COEFFICIENT_FRICTION_KINETIC;
	...
	string friction_values = ConvertO(fFriction[0]) + '\t' + ConvertO(fFriction[2]) + '\n';
	writeToFile("friction_values.txt",friction_values);
